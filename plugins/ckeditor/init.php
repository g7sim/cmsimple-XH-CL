...
/*
 * @version $Id: init.php 249 2017-04-28 23:14:44Z hi $
 *
 */
...
function ckeditor_filebrowser() {
    ...
    if (CMSIMPLE_XH_VERSION != '@CMSIMPLE_XH_VERSION@' && version_compare(CMSIMPLE_XH_VERSION, 'CMSimple_XH 1.7', 'lt')
    ) {
        $url = CMSIMPLE_ROOT .
                'plugins/filebrowser/editorbrowser.php?editor=ckeditor&prefix=' .
                CMSIMPLE_BASE;
    } else {  // CMSimple_XH v1.7 (r1518)
        $url = CMSIMPLE_ROOT .
                '?filebrowser=editorbrowser&editor=ckeditor&prefix=' .
                CMSIMPLE_BASE;
    }
...
function ckeditor_parseConfig($element = false, $config = false) {
    ...
    //include Fontawesome stylesheet, if activated
    if (ckeditor_isFontawesomeAvailable()) {
        $css .= ',"' . $pth['folder']['plugins'] . 'fa/css/font-awesome.min.css"';
        $faUrl = 'fontawesomeCss_url : "' . CMSIMPLE_ROOT . 'plugins/fa/css/font-awesome.min.css"';
        $temp = str_replace('//%fontawesomeCss_url%', $faUrl, $temp);
    }

    //$temp = str_replace('%BASE_HREF%', 'http://' . $_SERVER['HTTP_HOST'] . $sn, $temp);
    $temp = str_replace('%BASE_HREF%', CMSIMPLE_URL, $temp);
...
function include_ckeditor() {
    ...
    $hjs .='
        <script language="javascript" type="text/javascript" src="'
            . $pth['folder']['plugins'] . 'ckeditor/'
            . 'ckeditor/ckeditor.js"></script>'
            . PHP_EOL;
    $hjs .= '<script type="text/javascript">CKEDITOR.dtd.$removeEmpty[\'span\'] = false;</script>' . PHP_EOL;
    $hjs .= '<script type="text/javascript">CKEDITOR.dtd.$removeEmpty[\'i\'] = false;</script>' . PHP_EOL;

    $hjs .= '<script language="javascript" type="text/javascript" src="'
            . $pth['folder']['plugins']
            . 'ckeditor/init.js"></script>'
            . PHP_EOL;

    $pjs = '';
    //include Fontawesome-Plugin if activated
    if (ckeditor_isFontawesomeAvailable()) {
        $pjs .= 'CKEDITOR.plugins.addExternal(\'fontawesome\', \''
                . CMSIMPLE_ROOT . 'plugins/fa/editors/ckeditor/\');'
                . PHP_EOL;
    }
    $externalPlugins = ckeditor_getExternalPlugins();
    if (count($externalPlugins) > 0) {
        foreach ($externalPlugins as $plugin) {
            $pjs .= 'CKEDITOR.plugins.addExternal(\''
                    . $plugin . '\', \'../plugins_external/'
                    . $plugin . '/\');' . PHP_EOL;
        }
    }
    if ($pjs !== '') {
